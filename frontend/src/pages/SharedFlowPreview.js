...
const SharedFlowPreview = () => {
  ...
  const fetchWithRetry = async (url, options = {}, retries = 3, delay = 1000) => {
    try {
      ...
      // Se for erro de autenticação, não faça retry
      if (error.response && error.response.status === 401) {
        // Limpe o token inválido
  ...
  useEffect(() => {
    const fetchSharedFlow = async () => {
      try {
        ...
        try {
          // Verificar se há token disponível e obter headers apropriados
          const headers = {};
          ...
          if (apiError.response && apiError.response.status === 401) {
            // O token pode ter expirado, limpe-o
            SecureStorageService.clearToken();
          }
    ...
    <div className="min-h-screen h-screen w-full flex flex-col overflow-hidden bg-gray-100">
      ...
      <div className="py-2 bg-gray-100 border-t border-gray-200">
        <div className="text-center text-xs text-gray-500">
          Powered by <a href="/" className="text-primary hover:underline">SimulaChat</a>
        </div>
      </div>
    </div>
  );
