...
const Templates = () => {
  ...
  const fetchTemplatesFromAPI = async () => {
    try {
      ...
      const response = await axios.get(`${apiConfig.baseUrl}/templates`, {
        headers: {
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      ...
      try {
        ...
        // Tentar carregar da API primeiro se estiver autenticado
        if (isAuthenticated) {
          try {
          ...
          // Se não estiver autenticado, usar armazenamento local
          templatesData = StorageService.getTemplates();
        }
  ...
  const handleUseTemplate = async (templateId) => {
    try {
      ...
      // Tentar aplicar o template via API se autenticado
      if (isAuthenticated) {
        try {
          const authToken = localStorage.getItem('authToken');
          
          const response = await axios.post(`${apiConfig.baseUrl}/flows/from-template/${templateId}`, {}, {
            headers: {
              Authorization: `Bearer ${authToken}`
            }
          });
