    ...
    const fetchPlans = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          // Fallback para planos estáticos com novos preços
          setPlans([
            {
              id: 1,
              name: 'Básico',
        ...
        // Fallback para planos estáticos com novos preços em caso de erro
        setPlans([
          {
            id: 1,
            name: 'Básico',
  ...
  const handleSubscribe = async (planId) => {
    ...
    try {
      ...
      if (response.data.success && response.data.redirectUrl) {
        // Salvar o plano selecionado no localStorage para uso posterior
