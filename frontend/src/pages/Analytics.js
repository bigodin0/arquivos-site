    ...
    const fetchFlowsFromAPI = async () => {
      try {
        ...
        if (!token) {
          ...
          // Função segura para obter valores de estatísticas
          const getStatValue = (stats, path) => {
          ...
          // Função segura para calcular uso de templates
          const calculateTemplatesUsed = (stats) => {
            if (!stats || !stats.templates || !stats.templates.used) {
              return 0;
            }
            
            try {
              const values = Object.values(stats.templates.used);
              if (!Array.isArray(values)) {
                return 0;
              }
              return values.reduce((sum, count) => sum + (Number(count) || 0), 0);
            } catch (error) {
              console.error('Erro ao calcular templatesUsed:', error);
              return 0;
            }
          };
          ...
          const activitySummary = {
            ...
            totalMessages: savedFlows.reduce((count, flow) => count + (Array.isArray(flow.messages) ? flow.messages.length : 0), 0),
            flowsCreated: getStatValue(stats, 'flows.created'),
            flowsEdited: getStatValue(stats, 'flows.edited'),
            flowsShared: getStatValue(stats, 'flows.shared'),
            flowsViewed: getStatValue(stats, 'flows.views'),
            messagesCreated: getStatValue(stats, 'messages.sent'),
            templatesUsed: calculateTemplatesUsed(stats),
        ...
        if (response.data && response.data.success && Array.isArray(response.data.data)) {
          ...
          const activitySummary = {
            ...
            totalMessages: apiFlows.reduce((count, flow) => count + (Array.isArray(flow.messages) ? flow.messages.length : 0), 0),
            ...
            messagesCreated: apiFlows.reduce((count, flow) => count + (Array.isArray(flow.messages) ? flow.messages.length : 0), 0),
  ...
  return (
    <MainLayout title="Analytics">
      <Container>
          ...
          <div className="p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
              ...
              <Select
                ...
                onChange={(e) => {
                  const flowId = Number(e.target.value) || e.target.value;
            ...
            {selectedFlow && stats ? (
                          ...
                          {Array.isArray(stats.popularButtons) && stats.popularButtons.map((button, index) => (
                            <div key={index} className="flex justify-between py-2 border-b border-gray-200 last:border-0">
                              <div className="flex items-center">
                                ...
                                // Analytics.js continued from where the file was cut off

                                <span className="bg-teal-100 text-teal-800 text-xs px-1.5 py-0.5 rounded-full mr-2">
                                  {index + 1}
                                </span>
                                <span className="text-gray-700 text-sm font-medium truncate max-w-[180px]">
                                  {button.name}
                                </span>
                              </div>
                              <span className="text-gray-600 text-sm">{button.clicks} cliques</span>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg py-12 px-6 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-800 mb-2">Nenhum fluxo selecionado</h3>
                <p className="text-gray-600 mb-6 max-w-md mx-auto">
                  Selecione um fluxo acima para visualizar suas estatísticas de desempenho.
                </p>
                {flows.length === 0 && (
                  <Link 
                    to="/flows/new" 
        ...
        <AnalyticsCard>
          <div className="p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-6">Resumo de Atividade</h2>
            ...
            <Grid cols={1} md={2} lg={3} gap={6}>
              <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-gray-600 font-medium text-sm">Fluxos Criados</h3>
                    <p className="text-2xl font-bold text-gray-800">{activitySummary?.flowsCreated || 0}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-gray-600 font-medium text-sm">Fluxos Visualizados</h3>
                    <p className="text-2xl font-bold text-gray-800">{activitySummary?.flowsViewed || 0}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-gray-600 font-medium text-sm">Mensagens Criadas</h3>
                    <p className="text-2xl font-bold text-gray-800">{activitySummary?.messagesCreated || 0}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-gray-600 font-medium text-sm">Templates Utilizados</h3>
                    <p className="text-2xl font-bold text-gray-800">{activitySummary?.templatesUsed || 0}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-gray-600 font-medium text-sm">Última Atualização</h3>
                    <p className="text-sm font-medium text-gray-800">
                      {activitySummary?.lastUpdated ? 
                        new Date(activitySummary.lastUpdated).toLocaleString('pt-BR', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                          hour: '2-digit',
