...
const Dashboard = () => {
  ...
  const fetchFlowsFromAPI = async () => {
    try {
      ...
      // Verificar se temos headers de autenticação
      const headers = getAuthHeaders();
      
      if (!headers) {
        // Se não houver headers de autenticação, usar os fluxos locais como fallback
        const localFlows = StorageService.getFlows() || [];
        setFlows(localFlows);
        return;
      }
      
      // Fazer a chamada à API usando getAuthHeaders()
      const response = await axios.get(apiConfig.endpoints.flows, headers);
